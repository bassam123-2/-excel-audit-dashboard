...
@pytest.mark.integration
@pytest.mark.django_db
def test_frontend_restore_is_disabled(api_client, superuser, draft_dashboard, btc_company):
    login_and_select_company(api_client, "pytest_super", btc_company)
    api_client.post(
        f"/dashboards/{draft_dashboard.pk}/delete/",
        HTTP_X_REQUESTED_WITH="XMLHttpRequest",
...
@pytest.mark.integration
@pytest.mark.django_db
def test_member_can_soft_delete_own_draft(api_client, btc_company):
